...
\texttt{src/bankruptcy\_ml/config.py}. The other modules are organized by task,
including data validation, exploratory analysis, preprocessing, company-level
splitting, model fitting, evaluation, threshold analysis, PCA, interpretation,
and visualization. Files in \texttt{tasks/} connect these stages in the build
process, while files in \texttt{tests/} check the main data and modelling steps.
...
The build command creates the processed data, fitted models, result tables, and
figures in the correct order. The test command
...
folder contains simplified copies and summaries of the existing results. It
does not run the models a second time.
