...
\begin{abstract}
This paper studies company-year bankruptcy-risk classification using financial
statement variables. The task is to distinguish failed and alive observations,
without claiming causality or a defined future forecast horizon. The data contain
...
no failures. Ranking performance is assessed mainly with PR-AUC, implemented as
average precision. Failed-class recall and F1 are evaluated at the default
...
balanced accuracy and was selected ex ante using validation PR-AUC. PCA retains
much of the predictor variance but does not clearly improve validation
performance. Overall, the models are useful as screening and ranking tools, but
the results are not causal estimates, the models are not production-ready, and
no model is best for every objective.
\end{abstract}
...
interactions improve performance. Together, these models move from a transparent
parametric classifier to more flexible tree-based methods. This reflects the
interpretability--flexibility and bias--variance trade-offs emphasized in
statistical learning \citep{james2023}. A PCA plus Logistic Regression
...
project. First, it builds a reproducible comparison of interpretable and
tree-based classifiers while avoiding direct company overlap between training
and test data. Second, it evaluates the models with metrics that focus on the
minority failed class and uses an untouched final test set for the main
comparison. Third, it links the statistical results to the financial screening
problem through confusion-matrix counts, validation-based threshold trade-offs,
Logistic Regression coefficients, and tree-based feature importance. The results
are interpreted as predictive associations rather than causal effects. The study
is an educational model comparison, not a production system for automated credit
...
calendar years overlap with the training years. The results should therefore be
read as evidence about generalization across companies under this split, not as
a real-time forecast of a later period.
...
The models therefore use reported financial levels rather than engineered
...
in 2018. This pattern should be interpreted with care. It may reflect economic
conditions, changes in the companies included in the dataset, or the way status
is represented. The figure describes the sample. It is not evidence of a causal
time trend.
...
variables cause failure. They are simple group comparisons before controlling
...
The validation rows are not added back for a final refit. This is conservative
because the final models are fitted with fewer observations. It is also less
...
methods can reduce variance when predictors are correlated, although they add
some bias \citep{james2023}.
...
tree model in the comparison. It repeatedly divides the predictor space into
smaller regions. Its depth and minimum leaf size control complexity. A deep tree
...
matter, but their costs are not assumed to be equal.
...
how well a model ranks a randomly chosen failed observation above a randomly
chosen alive observation across all thresholds. However, ROC curves can look
...
sensitivity check, not as a deployed policy. A real decision rule would also
...
at 0.159 and the highest failed-class F1-score at 0.232. This is a final-test
observation, not a new model-selection rule. Gradient Boosting has the highest
...
comparison is metric-dependent and should be read as a set of trade-offs.
...
consistent with balanced class weighting. Failed observations receive more
...
contain predictive information, but the low precision levels show that
bankruptcy-risk classification remains difficult.
...
fitting, their coefficient magnitudes can be compared on a common scale.
...
These interpretations hold the other predictors fixed.
...
bankruptcy. It may reflect the way gross profit moves together with size,
revenue, costs, and balance-sheet variables in the fitted prediction model.
...
individual coefficients. Their role here is predictive interpretation.
...
that a variable was more useful for splitting observations within that fitted
model. Unlike a logistic coefficient, this measure does not show whether higher
...
can be divided across related variables. The figure is therefore best used as a
summary of the fitted models, not as a definitive ranking of economic causes.
...
compressed without losing much predictive information. The first few components
...
Explained variance is not the same as classification performance.
...
versions with ten or twelve components are therefore similar, not clearly
better. PCA can reduce the input dimension. However, strong compression to two
or three components removes information that helps separate failed and alive
observations.
...
small validation gain does not provide a strong reason to accept that loss of
interpretability. The PCA exercise is still useful because it shows a clear
lesson: predictor variance is not the same as predictive value. All PCA
comparisons are validation results only. They do not create additional
final-test claims.
...
final comparison more credible. Finally, the majority baseline and failed-class
...
therefore test generalization to unseen companies within a similar historical
period, not performance in a later economic period. The changing annual failure
...
These issues limit external generalization.
...
project understandable but may leave better settings unexplored. The saved
final models are also fitted on the inner model-training portion and are not
refitted after validation. This is conservative because some available training
observations are not used in the final fit. It is also less data-efficient. A
...
contain useful classification and ranking information under the company-level
split. At the same time, bankruptcy classification remains difficult, and no
model is best for every objective.
...
distinction is important because the final test set is used for evaluation, not
for retrospective model selection.

The additional analyses support the same trade-off. Validation threshold
analysis showed that higher recall requires lower precision and more review
effort. PCA reduced the number of inputs but did not provide a clear validation
advantage. Logistic Regression coefficients and tree importance helped interpret
the fitted models, especially around valuation, profitability, liquidity, and
debt variables, but these patterns are not causal evidence. Overall, the models
are useful for ranking or screening companies in an educational setting, not for
automatic decisions or production use. A stronger future study should define a
forward prediction horizon, use chronological evaluation, assess calibration and
uncertainty, and choose thresholds using financial costs and review capacity.
...
Table~\ref{tab:model-settings} summarizes the model specifications used in the
...
Figure~\ref{fig:threshold-tradeoff} shows the corresponding validation
precision, recall, and failed-class F1 patterns across candidate thresholds.
...
Figure~\ref{fig:pca-variance} shows how quickly the principal components capture
variation in the standardized predictors.
